...
w2v.getNearest = getNearestSync;
w2v.getNearestSync = getNearestSync;
w2v.getNearestAsync = getNearestAsync;
module.exports = w2v;

function getNearestSync(v, returnType = "array")
{
  var result = [];
  var ts = v, t, s;
  this.checkModelFile();
  ...
  var child;
  if(this.mime === 'text/plain') child = spawnSync('./getNearest', ['-f', this.modelFile, '-v', ts], {cwd: utils.SRC_DIR});
  else child = spawnSync('./getNearest', ['-f', this.modelFile, '-v', ts, '-b', 0], {cwd: utils.SRC_DIR});

	var errorText = child.stderr.toString().trim();
	if (errorText) {
	  console.log('Fatal error from ./getNearest.');
...
function getNearestAsync(word = "臺灣", callback)
{
  var result = {};
  if(!_.isString(word))
	{
