...
function getSimilarSync(word = "臺灣", returnType = "object")
{
  var result = [];
  if(!_.isString(word))
	{
  ...
  this.checkModelFile();
  var stdout = execSync('./getSimilar -f '+this.modelFile+' -w '+word, {cwd: utils.SRC_DIR, encoding: 'utf-8'});
...
function getSimilarAsync(word = "臺灣")
{
  if(!_.isString(word))
	{
		word = "臺灣";// `${data}`  //`.......   exitCode: ${exitCode}`
